...
class TreeExecution:
    ...
    def lightgbm_optuna_holdout(self):
        X=self.df.drop(columns=[f"{self.y_name}"])
        y=self.df[f"{self.y_name}"]
        prefecture_labels=X["都道府県名"]
        #threshold=0.074
        #cv=KFold(n_splits=3,shuffle=True)
        self.train,self.test=train_test_split(self.df,stratify=self.df["都道府県名"],test_size=0.12)
        print("self.df.shape",self.df.shape)
        print("self.train.shape",self.train.shape)
        print("self.test.shape",self.test.shape)
    ...
    def lightgbm_optuna(self):
        ...
        def objective(trial):
            ...
            "n_estimators": trial.suggest_int("n_estimators", 1500, 50000),
            "colsample_bytree": trial.suggest_float("colsample_bytree", 0.2, 1),#0.5
            "colsample_bynode": trial.suggest_float("colsample_bynode", 0.2, 1),#0.5
            "min_child_samples": trial.suggest_int("min_child_samples", 100, 1000),
            "min_child_weight": trial.suggest_float("min_child_weight", 0.1, 10),
            ...
            for idx_train, idx_valid in cv.split(X, prefecture_labels):#   Because it takes a long time to divide the data set, 
                ...
                model.fit(
                    X_train, y_train,
                    eval_set = [(X_valid, y_valid)],
                    callbacks = [lgb.log_evaluation(750), lgb.early_stopping(100)] )
                fitted_models_lgbm.append(model)
                y_pred_valid = model.predict(X_valid)
                oof_pred_lgbm[idx_valid]=y_pred_valid
                auc_score = mean_absolute_error(y_valid, y_pred_valid)
                cv_scores_lgbm.append(auc_score)

            #print("CV AUC scores: ", cv_scores_cat)
            #print("Maximum CV AUC score: ", max(cv_scores_cat))
            trial.set_user_attr("models",fitted_models_lgbm)
            print("CV AUC scores: ", cv_scores_lgbm)
            print("Maximum CV AUC score: ", min(cv_scores_lgbm))
            print("Mean CV AUC score: ",mean(cv_scores_lgbm))
            ...
            return mean(cv_scores_lgbm)
        
        sampler = optuna.samplers.TPESampler(multivariate=True)
        study=optuna.create_study(direction="minimize",sampler=sampler)
        study.optimize(objective,n_trials=65)

        fitted_models_lgbm=study.best_trial.user_attrs["models"]
        return fitted_models_lgbm
    ...
    def catboost_optuna_holdout(self):
        ...
        def objective(trial):
            # 初回の試行の場合、特定のパラメータを設定する
            if trial.trial_id==0:
                params={'iterations': 16321,
                        'learning_rate': 0.02141285001219377,
                        'depth': 12,
                        'l2_leaf_reg': 1.1080618593367644, 
                        'border_count': 226, 
                ...
                params = {
                    "iterations": trial.suggest_int("iterations", 5000, 21000),  # イテレーション数
                    "learning_rate": trial.suggest_uniform("learning_rate", 0.001, 0.12),  # 学習率
                    ...
                    "od_wait": trial.suggest_int("od_wait", 20, 500),  # 過学習検出の待機イテレーション数
                    ...
                    #"colsample_bylevel": trial.suggest_float("colsample_bylevel", 0.01, 1.0),  # 各レベルで使用する特徴量の割合
